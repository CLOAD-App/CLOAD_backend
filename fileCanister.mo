...
shared ({caller = owner}) persistent actor class Canister() = this {
  ...
  private var storeToData_s: [(Text, FileChunkPath)] = [];
  private transient let storeToData = HashMap.fromIter<Text, FileChunkPath>(storeToData_s.vals(), 0, Text.equal, Text.hash);
 ...
 // Query file by ChunkID
  ...
  // Cleanup storage: remove entries not in fileList
